 ...
 * @link       http://pear.php.net/pepr/pepr-proposal-show.php?id=212
 */
class tx_pbimagegraph_Canvas_PDF extends tx_pbimagegraph_Canvas
{
     ...
     */
    function tx_pbimagegraph_Canvas_PDF($param)
    {
        if (isset($param['page'])) {
        ...
        parent::tx_pbimagegraph_Canvas($param);
        ...
        if ((isset($param['pdf'])) && (is_resource($param['pdf']))) {
            ...
            pdf_set_info($this->_pdf, 'Creator', (isset($param['creator']) ? $param['creator'] : 'PEAR::tx_pbimagegraph_Canvas'));
            pdf_set_info($this->_pdf, 'Author', (isset($param['author']) ? $param['author'] : 'Jesper Veggerby'));
            pdf_set_info($this->_pdf, 'Title', (isset($param['title']) ? $param['title'] : 'tx_pbimagegraph_Canvas'));
        }
    ...
    function _color($color = false)
    {
        if (($color === false) || ($color === 'opague') || ($color === 'transparent')) {
            ...
            $color = tx_pbimagegraph_Canvas_Color::color2RGB($color);
            $color[0] = $color[0]/255;
            $color[1] = $color[1]/255;
            $color[2] = $color[2]/255;
            return $color;
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/pbimagegraph/Image/Canvas/class.tx_pbimagegraph_canvas_pdf.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/pbimagegraph/Image/Canvas/class.tx_pbimagegraph_canvas_pdf.php']);
}
?>