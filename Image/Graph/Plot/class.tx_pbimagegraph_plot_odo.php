 ...
 * @link       http://pear.php.net/package/tx_pbimagegraph
 */
class tx_pbimagegraph_Plot_Odo extends tx_pbimagegraph_Plot
{
    /**
     ...
     * @param tx_pbimagegraph_Dataset $dataset The data set (value containter) to
     *   plot or an array of datasets
     *   {@link tx_pbimagegraph_Legend}
     */
    function tx_pbimagegraph_Plot_Odo(&$dataset)
    {
        parent::tx_pbimagegraph_Plot($dataset);
    ...
    function _initialize()
    {
        ...
        $dimensions = tx_pbimagegraph_Tool::calculateArcDimensionAndCenter($v1, $v2);
     ...
     * @param tx_pbimagegraph_Marker $marker The marker to set for arrow marker
     */
     ...
     *
     * @param tx_pbimagegraph_Line $lineStyle The line style of the Arrow
     ...
     *
     * @param tx_pbimagegraph_Fill $fillStyle The fill style of the arrows
    ...
    function _drawArrow()
    {
        ...
        foreach ($keys as $key) {
            ...
            while ($point = $dataset->_next()) {
                ...
                list($xr,$yr) = tx_pbimagegraph_tool::rotate($this->_centerX + $this->_arrowLength * $this->_radius / 100, $this->_centerY, $this->_centerX, $this->_centerY, $deg);
                $this->_canvas->addVertex(array('x' => $xr, 'y' => $yr));
                list($xr,$yr) = tx_pbimagegraph_tool::rotate($this->_centerX, $this->_centerY - $this->_arrowWidth * $this->_radius/100, $this->_centerX, $this->_centerY, $deg);
                $this->_canvas->addVertex(array('x' => $xr, 'y' => $yr));
                list($xr,$yr) = tx_pbimagegraph_tool::rotate($this->_centerX - $this->_arrowWidth * $this->_radius / 100, $this->_centerY, $this->_centerX, $this->_centerY, $deg);
                $this->_canvas->addVertex(array('x' => $xr, 'y' => $yr));
                list($xr,$yr) = tx_pbimagegraph_tool::rotate($this->_centerX,$this->_centerY + $this->_arrowWidth * $this->_radius / 100, $this->_centerX, $this->_centerY, $deg);
                $this->_canvas->addVertex(array('x' => $xr, 'y' => $yr));
     ...
     *
     * @param tx_pbimagegraph_Fill $fillStyle The fill style of the range
     * @see tx_pbimagegraph_Fill
     * @access public
     */
    ...
    function _legendSample(&$param)
    {
        if (is_array($this->_dataset)) {
            ...
            if (is_a($this->_fillStyle, "tx_pbimagegraph_Fill")) {
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/pbimagegraph/Image/Graph/Plot/class.tx_pbimagegraph_plot_odo.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/pbimagegraph/Image/Graph/Plot/class.tx_pbimagegraph_plot_odo.php']);
}
?>