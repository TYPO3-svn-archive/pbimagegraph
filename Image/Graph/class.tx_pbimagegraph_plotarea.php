 ...
 * @link       http://pear.php.net/package/tx_pbimagegraph
 */
class tx_pbimagegraph_Plotarea extends tx_pbimagegraph_Layout
{
    ...
    /**
     * tx_pbimagegraph_Plotarea [Constructor]
     *
     ...
     */
    function tx_pbimagegraph_Plotarea($axisX = 'tx_pbimagegraph_Axis_Category', $axisY = 'tx_pbimagegraph_Axis', $direction = 'vertical')
    {
        ...
        $this->_axisX =& tx_pbimagegraph::factory($axisX, IMAGE_GRAPH_AXIS_X);
        $this->_axisX->_setParent($this);

        $this->_axisY =& tx_pbimagegraph::factory($axisY, IMAGE_GRAPH_AXIS_Y);
     ...
     *
     * @param tx_pbimagegraph_Line $lineStyle The line style of the border
     ...
     *
     * @param tx_pbimagegraph_Element $element The element to add
     ...
     * IMAGE_GRAPH_AXIS_Y)
     * @return tx_pbimagegraph_Element The added element
     * @see tx_pbimagegraph_Common::add()
     */
    function &add(& $element, $axis = IMAGE_GRAPH_AXIS_Y)    
    {
        ...
        {
            $this->_axisYSecondary =& tx_pbimagegraph::factory('axis', IMAGE_GRAPH_AXIS_Y_SECONDARY);
        ...
        } elseif (is_a($element, 'tx_pbimagegraph_Grid')) {
            switch ($axis) {
        ...
        } elseif (is_a($element, 'tx_pbimagegraph_Axis')) {
     ...
     *
     * @param tx_pbimagegraph_Plot $plot The plot that 'hold' the values
     * @access private
     */
    function _setExtrema(& $plot)
    {
        ...
        foreach ($keys as $key) {
            ...
            if (is_a($dataset, 'tx_pbimagegraph_Dataset')) {
                if (($this->_axisX != null) && (!$this->_axisX->_isNumeric())) {
     ...
     * tx_pbimagegraph_Plotarea::getAxis()})
     ...
     * @return tx_pbimagegraph_Axis The axis
     */
    ...
    {       
        if (is_array($this->_elements)) {
            ...
            foreach ($keys as $key) {
                ...
                if (is_a($element, 'tx_pbimagegraph_Plot')) {
                    if (((is_a($element, 'tx_pbimagegraph_Plot_Bar')) ||
                        (is_a($element, 'tx_pbimagegraph_Plot_Step')) ||
                        (is_a($element, 'tx_pbimagegraph_Plot_Dot')) ||
                        (is_a($element, 'tx_pbimagegraph_Plot_CandleStick')) ||
                        (is_a($element, 'tx_pbimagegraph_Plot_BoxWhisker')) ||
                        (is_a($element, 'tx_pbimagegraph_Plot_Impulse'))) &&
                        ($this->_axisX != null))
                    {
        ...
        tx_pbimagegraph_Element::_updateCoords();
/*
        if ($this->_axisX != null) {
            $this->_axisX->_updateCoords();
        }
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/pbimagegraph/Image/Graph/class.tx_pbimagegraph_plotarea.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/pbimagegraph/Image/Graph/class.tx_pbimagegraph_plotarea.php']);
}
?>